      ...
      {Array.from({ length: 7 }).map((_, index) => (
        ...
        >
          <mesh
            position={[
              (index % 3) * 1.05 - 1.05,
              Math.floor(index / 3) * 0.82 - 0.8,
              0,
            ]}
          >
            <boxGeometry args={[0.82, 0.72 + (index % 2) * 0.45, 0.82]} />
            <meshStandardMaterial
              color={index % 2 ? "#facc15" : "#1d4ed8"}
              metalness={0.25}
              roughness={0.28}
            />
          </mesh>
...
export function ConstructionHero3D() {
  return (
    <div className="h-[clamp(18rem,45vh,28rem)] w-full overflow-hidden rounded-[2rem] border border-white/15 bg-white/10 shadow-glass backdrop-blur-xl">
